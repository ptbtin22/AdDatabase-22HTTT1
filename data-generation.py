...
def generate_area_data(num_records):
    ...
    for i in range(1, num_records + 1):
        ...
        area_name = fake.city()
        print(
            f"INSERT INTO AREA (ID_Area, AreaName) VALUES ('{id_area}', '{area_name}');"
        )
    return area_id_list
...
def generate_department_data(department_names=departments):
    ...
    for i, department_name in enumerate(department_names, start=1):
        ...
        print(
            f"INSERT INTO DEPARTMENT (ID_Department, DepartmentName, Salary) VALUES ({id_department}, '{department_name}', {salary});"
        )
...
def generate_employee_data(num_records, department_id_list):
    ...
    for i in range(1, num_records + 1):
        ...
        print(
            f"INSERT INTO EMPLOYEE (ID_Employee, EmployeeName, DoB, PhoneNumber, Gender, ID_Department) VALUES ({id_employee}, '{name}', '{dob}', '{phone}', '{gender}', {department_id});"
        )
...
def generate_branch_data(
    ...
    for i in range(1, num_records + 1):
        ...
        print(
            f"INSERT INTO BRANCH (ID_Branch, BranchName, Address, PhoneNumber, OpeningHour, CloseHour, ID_Area, ID_Employee) "
...
def generate_parking_data(num_records, branch_id_list):
    ...
    for i in range(1, num_records + 1):
        ...
        print(
            f"INSERT INTO PARKING (ID_Parking, Address, OpeningHour, CloseHour, ParkingType, ID_Branch) "
...
def generate_food_type_data(sushi_food_types):
    ...
    for i, food_type in enumerate(sushi_food_types, start=1):
        ...
        print(
            f"INSERT INTO FOOD_TYPE (ID_Type, TypeName) VALUES ({id_food_type}, '{food_type}');"
        )
...
def generate_food_item_data(food_type_id_list, sushi_food_items):
    ...
    for food_type in food_type_id_list:
        ...
        for food_item in enumerate(food_items, start=1):
            ...
            print(
                f"INSERT INTO FOOD_ITEM (ID_Food, FoodName, Price, isAreaRestricted, DeliverySafe, ID_Type) VALUES ({id_food_item}, '{food_item}', {price}, {isAreaRestricted}, {deliverySafe}, {food_type_id});"
            )
            id_food_count += 1
    return {
...
def generate_area_specific_food_data(
    ...
    for i in range(1, num_records + 1):
        ...
        print(
            f"INSERT INTO AREA_SPECIFIC_FOOD (ID_Area, ID_Food) VALUES ({id_area}, {id_food});"
        )
...
def generate_branch_food_data(
    ...
    for i in range(1, num_records + 1):
        ...
        print(
            f"INSERT INTO BRANCH_FOOD (ID_BranchFood, Available, ID_Branch, ID_Food) VALUES ({id_branchfood}, {available}, {id_branch}, {id_food});"
        )
    return generated_pairs


def generate_branch_food_data2(
    branch_id_list,
    food_item_id_list,
    food_item_id_restricted,
    branch_area_list,
    ...
    return generated_pairs, branch_foods
...
def generate_mem_level_data(mem_levels):
    ...
    for i, mem_level in enumerate(mem_levels, start=1):
        ...
        print(
            f"INSERT INTO MEMBERSHIP_LEVEL (ID_MemLevel, MemLevelName, Discount) VALUES ({id_mem_level}, '{mem_level}', {discount});"
        )
...
def generate_membership_data(num_records, mem_level_id_list, id_employee_list):
    ...
    for i in range(1, num_records + 1):
        ...
        id_level = 1
        print(
            f"INSERT INTO MEMBERSHIP (ID_Card, Status, DateCreated, Point, ID_Level) VALUES ({id_card}, {status}, '{date_created}', {point}, {id_level});"
        )
    return list_card
...
def generate_customer_data(list_card):
    ...
    for i in range(1, len(list_card) + 1):
        ...
        id_card = i
        print(
            f"INSERT INTO CUSTOMER (ID_Customer, CustomerName, PhoneNumber, Email, Gender, SSID, ID_Card) VALUES ({id_customer}, '{name}', '{phone}', '{email}', '{gender}', '{ssid}', {id_card});"
        )
...
def generate_review_data(num_records):
    ...
    for i in range(1, num_records + 1):
        ...
        print(
            f"INSERT INTO REVIEW (ID_Review, ServiceScore, FoodScore, PriceScore, Comment) VALUES ({id_review}, {service_score}, {food_score}, {price_score}, '{comment}');"
        )
...
if __name__ == "__main__":
    ...
    area_specific_pairs = generate_area_specific_food_data(
        food_item_id_restricted, area_id_list, 5
    )
    print("-- Generate data for Branch Food table:")
    branch_food_pairs = generate_branch_food_data(
        branch_id_list,
        food_item_id_list,
        food_item_id_restricted,
        branch_area_list,
    ...
    card_list = generate_membership_data(
        no_of_branches, mem_level_id_list, employee_id_list
    )
